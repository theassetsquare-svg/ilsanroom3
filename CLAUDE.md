...
- Plain HTML site! NO REACT! NO VITE! NO JSX! NO npm! Just index.html + style.css + script.js! If you create React/Vite project = WRONG! Delete and start over with plain HTML! No React! No Vite! No JSX! Simple index.html + CSS + JS only!
FIRST! Before anything else, DELETE all React/Vite files:
rm -rf src/ node_modules/ dist/
rm -f vite.config.ts vite.config.js tsconfig.json tsconfig.node.json
rm -f package.json package-lock.json
rm -f postcss.config.js tailwind.config.js
Delete the old index.html too! Create fresh one from scratch.
This project was React before. Convert to Plain HTML NOW!
...
## NEVER
- NEVER show prices! No 가격! No 분양가! No 입장료! No 요금! Landing site = information only!
